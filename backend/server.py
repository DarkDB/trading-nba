...
async def get_sigma(user=Depends(get_current_user)):
    ...
    # Get games that have features computed
    games = await db.games.find(query, {"_id": 0}).sort("game_date", 1).to_list(10000)
    ...
    for game in games:
        ...
        feature_doc = await db.game_features.find_one({"game_id": game_id}, {"_id": 0})
        if not feature_doc:
            continue
        
        # Extract features
        X = np.array([[feature_doc.get(col, 0) for col in feature_cols]])
        
        # Check for all-zero features (would give constant prediction)
        if np.all(X == 0):
            continue
    ...
    if len(residuals) < 50:
        raise HTTPException(status_code=400, detail=f"Insufficient data: only {len(residuals)} games with features")
    ...
    flags = []
    if sigma_global < 8:
        flags.append("WARNING: sigma < 8 (unusually low variance)")
    if sigma_global > 20:
        flags.append("WARNING: sigma > 20 (unusually high variance)")
...
async def get_sigma(user=Depends(get_current_user)):
    ...
    return {
        "sigma_global": OPERATIONAL_CONFIG["calibration"]["sigma_global"],
        "sigma_source": OPERATIONAL_CONFIG["calibration"]["sigma_source"],
        "message": "Using default sigma. Run /admin/model/sigma/recompute to calculate from historical data."
    }
