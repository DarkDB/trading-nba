...
def test_generate_picks_returns_strategy_metadata(monkeypatch):
    ...
    assert len(response["operational_picks"]) == 0


def test_generate_picks_final_decision_is_explicit_with_max_trim(monkeypatch):
    fake_db = FakeDB()
    fake_db.performance_daily = FakeCollection(one={
        "roi_rolling_20": -0.01,
        "roi_rolling_50": -0.02,
        "n_clv_valid": 25,
        "market_beating_rate_valid": 0.40,
        "mean_clv_valid": -0.1,
        "max_drawdown_total": 0.1,
        "max_drawdown_total_units": 2.0,
        "context_scope": "global",
    })
    fake_db.upcoming_events = FakeCollection(docs=[
        {
            "event_id": "evt-1",
            "home_team": "Memphis Grizzlies",
            "away_team": "Boston Celtics",
            "commence_time": "2026-03-21T00:10:00+00:00",
            "status": "pending",
        },
        {
            "event_id": "evt-2",
            ...
            "commence_time": "2026-03-21T00:20:00+00:00",
            "status": "pending",
        },
    ])
    fake_db.market_lines = FakeCollection(docs=[
        {"event_id": "evt-1", "bookmaker_key": "pinnacle", "spread_point_home": 15.0, "price_home_decimal": 1.91, "price_away_decimal": 1.91},
        {"event_id": "evt-2", "bookmaker_key": "pinnacle", "spread_point_home": 18.0, "price_home_decimal": 1.91, "price_away_decimal": 1.91},
    ])
    monkeypatch.setattr(server, "db", fake_db)
    ...
    monkeypatch.setattr(server, "select_reference_line", lambda lines, require_pinnacle=True: lines[0] if lines else None)
    monkeypatch.setattr(server, "format_local_time", lambda ts: ts)
    monkeypatch.setattr(server, "generate_recommended_bet_string", lambda *_args, **_kwargs: "HOME +15.0")
    monkeypatch.setattr(server, "generate_explanation", lambda *_args, **_kwargs: "test")
    monkeypatch.setattr(server, "calculate_p_cover_vs_market", lambda *_args, **_kwargs: (0.59, 0.2))
    monkeypatch.setattr(server, "calculate_ev", lambda p, price: p * (price - 1) - (1 - p))
    monkeypatch.setattr(server, "calculate_signal_ev", lambda _ev: "green")
    monkeypatch.setattr(server, "calculate_signal", lambda _edge: "green")

    response = asyncio.run(server.generate_picks(user={"id": "u-1", "email": "ops@test.com"}))
    assert response["post_filter_counts"]["A"] == 1
    assert len(response["operational_picks"]) == 1
    assert len(response["shadow_picks"]) == 1
    assert response["shadow_picks"][0]["final_decision"] == "shadow"
    assert response["shadow_picks"][0]["final_decision_reason"] == "max_picks_per_day_trim"
    assert response["shadow_picks"][0]["final_decision_layer"] == "max_picks_trim"
    assert all(p.get("strategy_exclusion_reason") != "other" for p in response["all_picks"] if p.get("passed_strategy_profile"))
